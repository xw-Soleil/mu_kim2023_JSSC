# ECO10 stage 2v: final transition fix round before stream-out.
#
# closure5 left 12 max_transition rows: the two worst (-0.07/-0.08) are the
# outputs of the BUFFD4 repeaters stage 2u itself inserted (insufficient
# drive for those loads) -> upsize them; HFSNET_5036 (-0.01) gets the same
# upsize-or-guarded-buffer treatment.  The remaining nine rows are -0.00
# at-the-limit flicker (<5 ps over an internal 0.5 ns guideline) that every
# reroute reshuffles; they are documented as waived in the 2026-08-04 worklog
# rather than chased further.  No short fixes this round.
...
set TRANS_FIX_NETS {
  u_impl/PDE_TRANSFIX_NET
  u_impl/PDE_TRANSFIX_NET_1_0
  u_impl/HFSNET_5036
}
set SHORT_FIX_NETS {}
...
link_block
puts "PDE_ECO10_S2V_BEGIN design_lib=$design_lib"
...
foreach net_name $TRANS_FIX_NETS {
  ...
  if {$target_ref ne ""} {
    ...
    puts "PDE_ECO10_S2V_SIZED net=$net_name $ref->$target_ref"
    incr fixed
    continue
  }
  ...
  puts "PDE_ECO10_S2V_BUFFERED net=$net_name driver_ref=$ref slack=$slack"
  incr fixed
}
puts "PDE_ECO10_S2V_FIX_SUMMARY fixed=$fixed waived=[llength $waived]"
foreach entry $waived {
  puts "PDE_ECO10_S2V_WAIVED [lindex $entry 0] reason=[lindex $entry 1]"
}
...
foreach net_name $SHORT_FIX_NETS {
  ...
  puts "PDE_ECO10_S2V_ROUTES_REMOVED net=$net_name"
}
...
puts "PDE_ECO10_S2V_ROUTE_ECO_OPEN_END"
route_eco -open_net_driven false \
  -reroute modified_nets_first_then_others \
  -reuse_existing_global_route true \
  -utilize_dangling_wires true \
  -max_detail_route_iterations 100
puts "PDE_ECO10_S2V_ROUTE_ECO_DRC_END"
...
puts "PDE_ECO10_S2V_FILLERS_AFTER count=[sizeof_collection [get_cells -quiet {xofiller!PDE_FILLER_!*}]]"
...
puts "PDE_ECO10_S2V_VERIFIED"

save_block
save_lib
puts "PDE_ECO10_S2V_DONE design_lib=$design_lib"
exit
