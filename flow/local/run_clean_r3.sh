...
export PDE_ECO10_REPORT_DIR=${PDE_ECO10_REPORT_DIR:-$ECO10_ROOT/reports/icc2/closure3}
...
for attempt in $(seq 1 $MAX_ATTEMPTS); do
  echo "=== clean r3 尝试 $attempt/$MAX_ATTEMPTS 开始 $(date) ==="
  "$SNPS_RUN" icc2_shell -f flow/local/clean_r3_lastnet.tcl 2>&1 | tee "$LOG"
  echo "=== clean r3 尝试 $attempt 结束 $(date) ==="

  if grep -qE "^PDE_CLEAN_R3_DONE" "$LOG"; then
    echo "RESULT: OK"
    exit 0
  fi

  if grep -qE "^PDE_CLEAN_R3_BEGIN" "$LOG"; then
