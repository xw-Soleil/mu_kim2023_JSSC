#!/bin/bash
# ECO10 stage 3：终版报告与 GDS/DEF/netlist/SPEF 写出（只读，不做优化）。
# 详见 eco10_stage3_writeout.tcl 头部注释。
#
#   distrobox enter synopsys-focal -- flow/local/run_eco10_stage3.sh
#
# snps_no_udev.sh 绕开已知的 SCL udev 崩溃；启动重试继续作为保护，
# 判据是 '^PDE_ECO10_S3_BEGIN'。
set -o pipefail
LOCAL_DIR=$(dirname "$(readlink -f "$0")")
source "$LOCAL_DIR/env_local.sh"
SNPS_RUN=$LOCAL_DIR/snps_no_udev.sh

ECO10_ROOT=$PDE_REPO_ROOT/flow/local_runs/full_clean_20260804/icc2
...
export PDE_ECO10_RESULT_DIR=${PDE_ECO10_RESULT_DIR:-$ECO10_ROOT/results/icc2}

LOG=$ECO10_ROOT/reports/icc2/final.console.log
MAX_ATTEMPTS=25
mkdir -p "$(dirname "$LOG")"
...
for attempt in $(seq 1 $MAX_ATTEMPTS); do
  echo "=== eco10 stage3 尝试 $attempt/$MAX_ATTEMPTS 开始 $(date) ==="
  "$SNPS_RUN" icc2_shell -f flow/local/eco10_stage3_writeout.tcl 2>&1 | tee "$LOG"
  echo "=== eco10 stage3 尝试 $attempt 结束 $(date) ==="

  if grep -qE "^PDE_ECO10_S3_DONE" "$LOG"; then
    echo "RESULT: OK"
    exit 0
  fi

  if grep -qE "^PDE_ECO10_S3_BEGIN" "$LOG"; then
