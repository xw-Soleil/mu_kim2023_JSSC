#!/bin/bash
# ECO10 第一阶段：在 eco03 的副本上显式拆分 clock uncertainty 并存盘。
# 详见 eco10_stage1_constraints.tcl 头部注释。
#
#   distrobox enter synopsys-focal -- flow/local/run_eco10_stage1.sh
#
# snps_no_udev.sh 绕开已知的 SCL udev 崩溃；启动重试继续作为保护，
# 判据是 '^PDE_ECO10_S1_BEGIN'。
set -o pipefail
...
LOG=$ECO10_ROOT/reports/icc2/uncertainty_fix.console.log
MAX_ATTEMPTS=25
mkdir -p "$(dirname "$LOG")"
...
for attempt in $(seq 1 $MAX_ATTEMPTS); do
  echo "=== eco10 stage1 尝试 $attempt/$MAX_ATTEMPTS 开始 $(date) ==="
  "$SNPS_RUN" icc2_shell -f flow/local/eco10_stage1_constraints.tcl 2>&1 | tee "$LOG"
  echo "=== eco10 stage1 尝试 $attempt 结束 $(date) ==="

  if grep -qE "^PDE_ECO10_S1_DONE" "$LOG"; then
    echo "RESULT: OK"
    exit 0
  fi

  if grep -qE "^PDE_ECO10_S1_BEGIN" "$LOG"; then
