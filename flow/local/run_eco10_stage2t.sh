...
for attempt in $(seq 1 $MAX_ATTEMPTS); do
  echo "=== eco10 stage2t 尝试 $attempt/$MAX_ATTEMPTS 开始 $(date) ==="
  "$SNPS_RUN" icc2_shell -f flow/local/eco10_stage2t_targeted.tcl 2>&1 | tee "$LOG"
  echo "=== eco10 stage2t 尝试 $attempt 结束 $(date) ==="

  if grep -qE "^PDE_ECO10_S2T_DONE" "$LOG"; then
    echo "RESULT: OK"
    exit 0
  fi

  if grep -qE "^PDE_ECO10_S2T_BEGIN" "$LOG"; then
