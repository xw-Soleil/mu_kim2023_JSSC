# Floorplan and adjacent-layer M1-M6 power mesh.  The explicit intermediate
# stripes avoid fragile direct M1-to-M6 via stacks.
...
add_pdn_stripe -grid core_grid -layer M3 -width 0.40 -spacing 0.40 \
  -pitch 20.0 -offset 5.0 -starts_with POWER -snap_to_grid
add_pdn_stripe -grid core_grid -layer M4 -width 0.40 -spacing 0.40 \
...
pdngen -failed_via_report [file join $report_dir 00_pdn_v2_failed_vias.rpt]

report_design_area
write_db [file join $work_dir 00_floorplan_pdn_v2.odb]
write_def [file join $result_dir 00_floorplan_pdn_v2.def]
