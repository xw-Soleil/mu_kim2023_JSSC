# Rebuild the already-floorplanned PDN using a fixed foundry VIA12 master.
# This isolates a PSM issue with the design-generated 1x2 M1-M2 via.
...
# VIA12_2cut_E fits the 0.10um horizontal M1 rail and the 0.40um vertical
# M2 stripe while satisfying the two-cut rule for the wide M2 stripe.
...
pdngen -failed_via_report [file join $report_dir 01_pdn_fixed_via_failed.rpt]

set failures 0
foreach net {VDD VSS} {
  psm::clear_solvers
  set rc [catch {check_power_grid -net $net -floorplanning \
    -error_file [file join $report_dir 01_pdn_fixed_${net}_open.rpt]} msg]
  puts "PDE_PDN_FIXED_SMOKE net=$net rc=$rc message={$msg}"
  if {$rc != 0} {incr failures}
}

write_db [file join $work_dir 01_pdn_fixed_via_smoke.odb]
write_def [file join $result_dir 01_pdn_fixed_via_smoke.def]
if {$failures != 0} {
  error "Fixed-via PDN connectivity failed for $failures nets"
}
puts "PDE_PDN_FIXED_VIA_SMOKE_PASS"
exit
