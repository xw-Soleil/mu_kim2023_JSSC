# Rebuild only the top-level VDD/VSS special wires, avoiding pdngen -ripup's
# ambiguous power-domain inference in this hierarchical netlist.
...
foreach net_name {VDD VSS} {
  ...
  foreach swire [$pg_net getSWires] {
...
global_connect
set_voltage_domain -name CORE_FIXED -power VDD -ground VSS
define_pdn_grid -name core_grid_fixed -voltage_domains {CORE_FIXED} -starts_with POWER
...
pdngen -failed_via_report [file join $report_dir 01_pdn_fixed_v2_failed.rpt]

set failures 0
foreach net {VDD VSS} {
  psm::clear_solvers
  set rc [catch {check_power_grid -net $net -floorplanning \
    -error_file [file join $report_dir 01_pdn_fixed_v2_${net}_open.rpt]} msg]
  puts "PDE_PDN_FIXED_V2_SMOKE net=$net rc=$rc message={$msg}"
  if {$rc != 0} {incr failures}
}

write_db [file join $work_dir 01_pdn_fixed_via_smoke_v2.odb]
write_def [file join $result_dir 01_pdn_fixed_via_smoke_v2.def]
if {$failures != 0} {
  error "Fixed-via PDN connectivity failed for $failures nets"
}
puts "PDE_PDN_FIXED_VIA_V2_SMOKE_PASS"
exit
