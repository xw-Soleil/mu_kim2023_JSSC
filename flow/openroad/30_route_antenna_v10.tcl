# Strict global route retry with 90% of M2-M5 and 95% of M6 capacity.
# The v9 80/90% attempt had 15 bins, each with exactly one-track overflow.
...
foreach net {VDD VSS} {
  ...
  check_power_grid -net $net \
    -error_file [file join $report_dir 30_pdn_v10_pregrt_${net}_open.rpt]
}
...
# No -allow_congestion: any residual overflow remains fatal.
global_route -guide_file [file join $result_dir 30_route_antenna_v10.guide] \
  -congestion_iterations 100 -congestion_report_iter_step 10 \
  -critical_nets_percentage 0 \
  -congestion_report_file [file join $report_dir 30_congestion_v10.rpt]
...
write_db [file join $work_dir 35_antenna_v10.odb]
write_def [file join $result_dir 35_antenna_v10.def]
write_sdc -no_timestamp [file join $result_dir 35_antenna_v10.sdc]
write_guides [file join $result_dir 35_antenna_v10_postrepair.guide]

set antenna_violations [check_antennas \
  -report_file [file join $report_dir 35_antenna_v10.rpt]]
...
foreach net {VDD VSS} {
  psm::clear_solvers
  check_power_grid -net $net \
    -error_file [file join $report_dir 35_pdn_v10_${net}_open.rpt]
}

puts "PDE_OPENROAD_ROUTE_ANTENNA_V10_PASS"
exit
