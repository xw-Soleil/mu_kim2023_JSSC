# Parallel detailed-route path that reuses the completed v21 FastRoute guide.
# It intentionally skips the expensive pre-DR antenna-repair pass; antenna
# closure remains a hard post-DR gate with bounded repair/reroute rounds.
...
set input_db [file join $work_dir 20_cts_v4.odb]
set sdc [file join $repo_root flow openroad pde_chip_top_safe_v2.sdc]
set guide [file join $result_dir 30_route_antenna_v21.guide]
...
# Guard the 203 hierarchy-created, zero-terminal supply nets from a known
# antenna-checker crash.  They are restored to ordinary empty nets before DEF.
set block [ord::get_db_block]
...
run_drt initial_v23 $report_dir
set antenna_violations [check_antennas \
  -report_file [file join $report_dir 40_v23_initial_antennas.rpt]]
...
while {$antenna_violations > 0 && $antenna_round < 3} {
  ...
  run_drt v23_postant_${antenna_round} $report_dir
  set antenna_violations [check_antennas \
    -report_file [file join $report_dir 40_v23_postant_${antenna_round}.rpt]]
...
foreach net {VDD VSS} {
  ...
  check_power_grid -net $net \
    -error_file [file join $report_dir 40_pdn_v23_prefill_${net}_open.rpt]
}

write_db [file join $work_dir 40_route_v23_prefill_antclosed.odb]
write_def -version 5.8 [file join $result_dir 40_route_v23_prefill_antclosed.def]
...
check_placement -verbose -disallow_one_site_gaps \
  -report_file_name [file join $report_dir 45_final_v23_placement.json]

foreach net {VDD VSS} {
  psm::clear_solvers
  check_power_grid -net $net \
    -error_file [file join $report_dir 45_pdn_v23_final_${net}_open.rpt]
}

estimate_parasitics -global_routing
check_setup -verbose > [file join $report_dir 45_v23_check_setup.rpt]
report_floating_nets -verbose > [file join $report_dir 45_v23_floating_nets.rpt]
report_checks -path_delay max -fields {slew cap input_pins nets fanout} \
  > [file join $report_dir 45_v23_timing_checks.rpt]
report_design_area

write_db [file join $work_dir 45_final_v23.odb]
write_def -version 5.8 [file join $result_dir 45_final_v23.def]
write_def -version 5.8 [file join $result_dir 40_route_v23.def]
write_verilog -sort -include_pwr_gnd \
  [file join $result_dir 45_final_v23_lvs.v]
write_verilog -sort -include_pwr_gnd -remove_cells $filler_masters \
  [file join $result_dir 45_final_v23_nofill.v]
write_sdc -no_timestamp [file join $result_dir 45_final_v23.sdc]
puts "PDE_OPENROAD_PHYSICAL_V23_HARD_PASS"
exit
