# Detailed routing, closure gates, and physical filler insertion.
...
set input_db [file join $work_dir 35_antenna_v8.odb]
set sdc [file join $repo_root flow results openroad 35_antenna_v8.sdc]
...
detailed_route \
  -output_maze [file join $report_dir 40_route_v8_maze.log] \
  -output_drc [file join $report_dir 40_route_v8_drc.rpt] \
  -output_cmap [file join $report_dir 40_route_v8_cmap.csv] \
  -output_guide_coverage [file join $report_dir 40_route_v8_guide_coverage.rpt] \
...
write_db [file join $work_dir 40_route_v8_prefill.odb]
write_def [file join $result_dir 40_route_v8_prefill.def]

set antenna_violations [check_antennas \
  -report_file [file join $report_dir 40_route_v8_antennas.rpt]]
...
foreach net {VDD VSS} {
  ...
  check_power_grid -net $net \
    -error_file [file join $report_dir 40_pdn_v8_prefill_${net}_open.rpt]
}
...
check_placement -verbose -disallow_one_site_gaps \
  -report_file_name [file join $report_dir 40_route_v8_placement.json]

foreach net {VDD VSS} {
  psm::clear_solvers
  check_power_grid -net $net \
    -error_file [file join $report_dir 40_pdn_v8_final_${net}_open.rpt]
}
...
redirect [file join $report_dir 40_route_v8_checks.rpt] {
...
puts "PDE_OPENROAD_DETAILED_ROUTE_V8_PASS"
exit
