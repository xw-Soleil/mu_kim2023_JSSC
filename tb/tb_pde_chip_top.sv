// ============================================================================
// tb_pde_chip_top.sv : narrow-interface chip-top regression
// ============================================================================
`timescale 1ns/1ps

module tb_pde_chip_top;
  localparam int NROW = 8;
  localparam int NCOL = 8;
  ...
  pde_chip_top #(
    ...
    .CONV_ON_SMALL(1'b1), .DYN_PREC(1'b1), .MAX_UPDATES(2048)
  ) dut (
    .clk(clk), .rst_n(rst_n),
  ...
  // Wide pde_top runs in parallel so scan order and every result bit can be
  // checked exactly, not merely against a numerical tolerance.
  logic [16*NCOL-1:0] ref_bn, ref_bs;
  logic [16*NROW-1:0] ref_bw, ref_be;
  logic [16*NPEALL-1:0] ref_lr, ref_lb, ref_ur, ref_ub;
  logic ref_start, ref_load_en, ref_busy, ref_done, ref_converged;
  logic [1:0] ref_prec;
  ...
  pde_top #(
    ...
    .scan_in(1'b0), .scan_out(), .busy(ref_busy), .done(ref_done),
    .converged(ref_converged), .prec_o(ref_prec),
  ...
  function automatic logic [15:0] reference_word(input integer logical_idx);
    ...
    begin
      ...
      if (((row + col) % 2) == 0)
        reference_word = ref_ur[16*pe_idx +: 16];
      else
        reference_word = ref_ub[16*pe_idx +: 16];
    end
  endfunction
  ...
  integer expected_idx, got_signed, delta, scan_last_count;

  initial begin
    cfg_valid = 0; cfg_write = 0; cfg_addr = 0; cfg_wdata = 0;
    ref_start = 0; ref_load_en = 0; ref_lr = 0; ref_lb = 0;
    ref_bn = 0; ref_bs = 0; ref_bw = 0; ref_be = 0;
    ...
    $display("\n=== pde_chip_top 8x8 narrow-interface regression ===");
    if (cfg_ready !== 1'b1) fail("cfg_ready is not high while idle");

    // Sticky illegal-write error and CONTROL[2] clear.
    cfg_write_word(16'h5000, 16'hdead);
    cfg_read_word(A_STATUS, status_word);
    if (!status_word[ST_CFG_ERROR]) fail("invalid write did not set cfg_error");
    cfg_write_word(A_CONTROL, 16'h0004);
    cfg_read_word(A_STATUS, status_word);
    if (status_word[ST_CFG_ERROR]) fail("clear_error did not clear cfg_error");

    cfg_read_word(A_NROW, lo_word);
    cfg_read_word(A_NCOL, hi_word);
    if (lo_word != NROW || hi_word != NCOL)
      fail($sformatf("dimension registers returned %0d x %0d", lo_word, hi_word));

    // 32 words/colour = 64 zero-residue writes; four boundaries x 8 words.
    ...
    cfg_read_word(A_STATUS, status_word);
    if (!status_word[ST_CFG_COMPLETE])
      fail("cfg_complete did not assert after all 96 writes");
    ...
    if (chip_updates == 0 || chip_cycles == 0) fail("counters remained zero");
      ...
      if (scan_valid) begin
        ...
        if (bit_in_word == 15) begin
          ...
          if (scan_shift !== reference_word(expected_idx))
            fail($sformatf("scan word %0d/grid %0d got %04h expected %04h",
          ...
          if (delta*10 > BND)
            fail($sformatf("grid %0d differs from 4096 by >10%%: %0d",
                           expected_idx, got_signed));
    ...
    if (scan_words != GRID_WORDS) fail("scan word count is not 64");
    if (scan_last_count != 1) fail("scan_last did not pulse exactly once");
    ...
    if (errors == 0) begin
      $display("tb_pde_chip_top : PASS\n");
      $finish;
    end else $fatal(1, "tb_pde_chip_top : FAIL (%0d errors)", errors);
  end

  initial begin
    #2_000_000;
    $fatal(1, "tb_pde_chip_top : TIMEOUT");
  end
endmodule
